...
PDF files are ignored by git. Text-based PDFs are parsed into section-like chunks when headings can be detected, with page metadata preserved on each chunk. The default parser profile keeps product-manual heading hints for compatibility; use the `generic` profile plus explicit heading hints for other document families. Scanned image-only PDFs need OCR before indexing. Use subdirectories per product, for example:
