...
- Local UX affordances: the page may render non-persistent feedback controls, follow-up question suggestions, staged loading copy, and richer empty states as client-only UI. These affordances must reuse the same `/qa/answer` ask flow and must not imply durable feedback storage unless a backend persistence contract is added.
