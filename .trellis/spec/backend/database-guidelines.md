...
- PDF rebuilds may write `meta["pdf_quality"]` as a bounded parser-quality summary with `documents`, `pages_total`, `pages_with_text`, `pages_missing_text`, `ocr_pages_created`, and low-cardinality `warning_counts`. It must not include raw PDF text, snippets, local absolute paths, full parser warning strings, or per-page extracted content.
...
- Unit/API: PDF parser quality summaries count native-text pages, missing-text pages, OCR-created pages, bounded parser warnings, diagnostics payloads, and browser diagnostics hints without changing chunk output.
